...
<Callout emoji="⚠️">
  The <code>usage</code> mode is currently not as efficient as Babel, iff you
  have a usage like `"foo"["a" + "t"]()`. SWC does not evaluate the expression{" "}
  <code>"a" + "t"</code> and will not include the{" "}
  <code>String.prototype.at</code> polyfill.
</Callout>
