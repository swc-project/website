...
Plugins can receive configuration values (read [this section](../publishing#using-the-npm-package) on how to pass the values to the plugin).
To read the configuration, call [get_transform_plugin_config()](https://rustdoc.swc.rs/swc_core/plugin/proxies/struct.TransformPluginProgramMetadata.html#method.get_transform_plugin_config) on the metadata passed to the plugin:

```rust
#[plugin_transform]
pub fn process_transform(program: Program, metadata: TransformPluginProgramMetadata) -> Program {
    // config is a String, so you'll have to parse it yourself.
